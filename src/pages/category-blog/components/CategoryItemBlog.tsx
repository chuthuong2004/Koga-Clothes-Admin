...
type CategoryItemProps = {
  ...
  handleAddItemsBreadcrumb: (newCategory: StoreCategoryBlog) => void;
  handleChoiceCategory: (category: StoreCategoryBlog) => void;
};

const CategoryItemBlog: React.FC<CategoryItemProps> = ({
  category,
  handleAddItemsBreadcrumb,
  handleChoiceCategory,
}) => {
  const { mutate } = useSWRConfig();
  const { loading, onDeleteCategoryBlog } = useCategoryBlog()
  const handleClickCategory = async () => {
    ...
    mutate(`ListCategoryBlog${category._id}`, { ...response });
  ...
  const showDeleteConfirm = () => {
    Modal.confirm({
      ...
      content: 'Bạn sẽ không thể khôi phục lại danh mục này.',
      okText: 'Yes',
      okType: 'danger',
      cancelText: 'No',
      centered: true,
      ...
      onOk() {
        onDeleteCategoryBlog(category._id, () => {
...
export default memo(CategoryItemBlog);
