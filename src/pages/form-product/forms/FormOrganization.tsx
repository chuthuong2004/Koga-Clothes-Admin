...
const FormOrganization = () => {
    const { data } = useSWR('GetAllBrandsInFormProduct', () => brandService.getAll({
        page: 1,
        limit: 10000,
        offset: 0
    }))
    const { data: categories } = useSWR('GetAllCategoriesInFormProduct', () => categoryService.getAll({
        page: 1,
        limit: 100000,
        offset: 0
    }))
    const { control, formState: { errors }, setValue } = useFormContext<FormCreateProduct>()
        ...
        <Card bordered={false}>
            <Space direction='vertical' className='w-full'>
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Select
                                ...
                                options={data ? data.docs.map(brand => ({
                                    value: brand._id,
                                    label: brand.name
                                })) : []}
                            />
                    ...
                    {
                        errors.brand &&
                        <Typography.Text type='danger' >{errors.brand?.message}</Typography.Text>
                    }
                </div>
                <div className='flex flex-col gap-2'>
                        ...
                        render={({ field }) => (
                            <>
                                        ...
                                        return (
                                            <span key={tag} style={{ display: 'inline-block' }}>
                                                <Tag
                                                    ...
                                                    onClose={(e) => {
                                                        e.preventDefault();
                                                        setValue("keywords", field.value.filter((item) => item !== tag))
                                                    }}
                                                >
                                                    {tag}
                                                </Tag>
                                ...
                                {inputVisible ? (
                                    ...
                                    <Tag onClick={showInput} style={tagPlusStyle} className='border-dashed p-4 w-full'>
